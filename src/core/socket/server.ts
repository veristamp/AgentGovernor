  ...
  private async handleMessage(line: string): Promise<JsonRpcResponse> {
    ...
    switch (request.method) {
      ...
      case "__capabilities__": {
        ...
        const identity = { orgId: this.context.orgId, roles: this.context.roles || [] };
        const result = await this.capabilityRegistry.search("", identity, { limit: 100 });
      ...
      case "__tool_search__": {
        ...
        try {
          ...
          const result = await this.capabilityRegistry.search(query, identity, { limit });
      ...
      case "__inspect__": {
        ...
        try {
          const capabilityId = String(request.params?.skill || request.params?.capabilityId || "");
          if (!capabilityId) {
            return createError(request.id, ErrorCodes.INVALID_PARAMS, "Missing capabilityId");
          }

          const identity = { orgId: this.context.orgId, roles: this.context.roles || [] };
          const result = await this.capabilityRegistry.load(capabilityId, identity);
      ...
      case "__discover__": {
        ...
        try {
          ...
          const minShared = Math.min(Number(request.params?.minSharedConcepts || 2), 5);
          const limit = Math.min(Number(request.params?.limit || 5), 10);

          if (!fromCapability) {
            return createError(request.id, ErrorCodes.INVALID_PARAMS, "Missing fromCapability");
          }
          ...
          if (!node?.nodePointer) {
            return createResponse(request.id, { error: `Capability not found: ${fromCapability}` });
          }

          const related = await engram.hubHop(node.nodePointer.id, minShared, limit);
    ...
    try {
      ...
      if (message.includes("No client found")) code = ErrorCodes.METHOD_NOT_FOUND;
      else if (message.includes("Unauthorized")) code = ErrorCodes.UNAUTHORIZED;
      else if (message.includes("Forbidden") || message.includes("policy")) code = ErrorCodes.POLICY_DENIED;
