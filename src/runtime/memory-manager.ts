...
export class MemoryManager {
  ...
  calculateImportance(msg: CoreMessage): MessageImportance {
    ...
    const acknowledgments = ['thanks', 'thank you', 'ok', 'okay', 'got it', 'i see', 'understood'];
  ...
  prepareMessages(params: {
    ...
    // Update stats
    this.stats.episodicMessages = result.length;
    this.stats.compressedMessages = messages.length - result.length;
  ...
  private hasCitations(content: string): boolean {
    return content.includes('[citation:') || 
