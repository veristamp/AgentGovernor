...
export class AnalyticsManager {
  ...
  recordCompression(
    ...
    const compressionRatio = originalMessages > 0
  ...
  getCacheSummary(): {
    ...
    const avgCacheHitRate = sessionsWithCache > 0
      ? totalCacheHitRate / sessionsWithCache
      : 0;
  ...
  getCompressionSummary(): {
    ...
    for (const session of this.sessions.values()) {
      for (const event of session.compressionEvents) {
        ...
        totalMessagesCompressed += event.originalMessages - event.compressedMessages;
        totalTokensSaved += event.tokensSaved;
        eventCount++;
      }
    }

    const avgCompressionRatio = eventCount > 0
      ? totalCompressionRatio / eventCount
      : 0;
