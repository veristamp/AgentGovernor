...
class FastLruCache {
  ...
  constructor(private maxSize: number = 1000, private defaultTtl: number = 3600000) {}
...
function generateKey(params: unknown, prefix = "llm:"): string {
  const hash = createHash("sha256").update(JSON.stringify(params)).digest("base64url").slice(0, 32);
  return `${prefix}${hash}`;
}

/** Wrap model with high-performance caching */
export function wrapCachedModel(model: LanguageModel, config: CacheConfig = {}): LanguageModel {
  ...
  return {
    ...
    doGenerate: async (options: any) => {
      ...
      if (!diskWrites.has(key)) {
        diskWrites.add(key);
        Bun.write(`.cache/llm/${key}.json`, JSON.stringify({ result, ts: Date.now() })).catch(() => {});
      }
