...
`;
		...
		if (!hasWorkflowMatch) {
			const scout = await this.spawnScout(
				"Summarize authentication methods implemented in src/auth",
				"Inspect source files under src/auth and summarize auth mechanisms.",
	...
	private async spawnScout(
		...
		const runtimeIdentity = {
			...
			id: `scout-${Date.now()}`,
			type: "agent" as const,
			sessionId: `scout-session-${Date.now()}`,
		};
		...
		const runtime = await createAgentRuntime(ctx, activeTools);
...
Use tools to inspect files and return a concise summary (not code).`;
		...
		const userPrompt = `Summarize the authentication methods implemented under src/auth. Use filesystem tools to list and read relevant files. Return 3-6 short bullet points and include keywords like JWT, OAuth, admin client, agent client if present.`;

		const { final } = await runGovernedLoop(ctx, runtime, system, userPrompt, {
			maxIterations: 10,
		});
		...
		const listResult = await mcp.executeAction(
			...
			},
			{
				...
				missionId: ctx.identity.sessionId,
			},
		);
		...
		const summaryPrompt = `You have direct access to the file contents below. Summarize the authentication methods implemented in src/auth. Do not ask for more files. Only use the provided content and cite file names when relevant.\n\n${fileContents.join("\n\n")}`;
