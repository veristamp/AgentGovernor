...
export function decodeJWT(token: string): JWTClaims | null {
    try {
        ...
        const paddedB64 = padding !== 4 ? payloadB64 + '='.repeat(padding) : payloadB64;
...
export function decodeJWTHeader(token: string): { alg?: string; kid?: string; typ?: string } | null {
    try {
        ...
        const paddedB64 = padding !== 4 ? headerB64 + '='.repeat(padding) : headerB64;
        const base64 = paddedB64.replace(/-/g, '+').replace(/_/g, '/');
 ...
 */
export function isJWTExpired(claims: JWTClaims, bufferSeconds: number = 30): boolean {
    if (!claims.exp) {
 ...
 */
export function checkJWTAudience(claims: JWTClaims, expectedAudience: string): boolean {
