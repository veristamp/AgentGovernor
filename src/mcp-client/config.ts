...
function parseServerConfig(raw: Record<string, unknown>, configPath: string): ServerConfig {
    ...
    if (raw.url) {
        ...
        type = 'sse';
    } else if (connectionType === 'streamable_http' || connectionType === 'http') {
    ...
    return {
        ...
        sseReadTimeout: (raw.sse_read_timeout || raw.sseReadTimeout) as number | undefined,
    };
}
