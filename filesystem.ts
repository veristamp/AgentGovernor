...
import { z } from 'zod';
import { mkdir, readdir, rename, stat, realpath } from 'node:fs/promises';
import { accessSync, constants as fsConstants, statSync, type Dirent } from 'node:fs';
...
const expandHome = (inputPath: string) =>
  inputPath.startsWith('~') ? path.join(os.homedir(), inputPath.slice(1)) : inputPath;

const normalizePath = (inputPath: string) => path.resolve(expandHome(inputPath));
...
const createUnifiedDiff = (original: string, modified: string, filepath: string) => {
  ...
  'read-file',
  {
    ...
    inputSchema: {
      ...
      encoding: z
        .string()
        .default('utf-8')
        .describe('"utf-8" for text files (default), "base64" for binary files (xlsx, images, pdf)'),
    },
  },
...
server.registerTool(
  'write-file',
  {
    ...
    inputSchema: {
      path: z.string().describe('Path to the file'),
      content: z.string().describe('Content to write (string or base64-encoded for binary)'),
      encoding: z.string().default('utf-8').describe('"utf-8" for text files (default), "base64" for binary files'),
  ...
  async ({ path: filePath, content, encoding, max_bytes }) => {
    ...
    if (encoding === 'base64') {
      ...
      return { content: [{ type: 'text', text: `Successfully wrote ${binary.length} bytes to ${filePath}` }] };
    }
...
server.registerTool(
  'edit-file',
  {
    ...
    inputSchema: {
      ...
      dry_run: z.boolean().default(true).describe('Whether to perform a dry run'),
    },
  },
  async ({ path: filePath, edits, dry_run }) => {
    ...
    for (const edit of edits) {
      ...
      for (let idx = 0; idx <= lines.length - oldLines.length; idx += 1) {
        const slice = lines.slice(idx, idx + oldLines.length).map((line) => line.trim());
...
server.registerTool(
  ...
  async ({ path: filePath }) => {
    ...
    return { content: [{ type: 'text', text: `Successfully created directory ${filePath}` }] };
  }
);
...
server.registerTool(
  ...
  async ({ path: filePath }) => {
    const validPath = await validatePath(filePath);
    const entries = (await readdir(validPath, { withFileTypes: true })) as Dirent[];
    const lines = entries.map((entry) => (entry.isDirectory() ? `[DIR] ${entry.name}` : `[FILE] ${entry.name}`));
...
server.registerTool(
  'directory-tree',
  {
    ...
    inputSchema: {
      ...
      max_nodes: z.number().default(5000).describe('Maximum number of nodes in the tree'),
    },
  },
  async ({ path: filePath, max_depth, max_nodes }) => {
    ...
    const buildTree = async (currentPath: string, depth: number): Promise<unknown[]> => {
      ...
      let entries: Dirent[];
      try {
        entries = (await readdir(currentPath, { withFileTypes: true })) as Dirent[];
      } catch (err) {
...
server.registerTool(
  'search-files',
  {
    ...
    inputSchema: {
      ...
      exclude_patterns: z.array(z.string()).optional().default([]).describe('Patterns to exclude'),
    },
  },
...
server.registerTool('list-allowed-directories', { description: 'Returns the list of directories this server can access.' }, async () => {
