    ...
    async def execute_action(self, action: Dict[str, Any]) -> str:
        ...
        try:
            if typ == "tool":
                ...
                resource = self.group.resources.get(name)
                if not resource:
                    return f"Resource '{name}' not found."
                content = resource.content.decode('utf-8') if resource.content else ""
                return content[:500] + "..." if len(content) > 500 else content
            elif typ == "prompt":
                prompt = self.group.prompts.get(name)
                if not prompt:
                    return f"Prompt '{name}' not found."
                texts = [c.text for msg in prompt.messages for c in msg.content if hasattr(c, 'text')]
