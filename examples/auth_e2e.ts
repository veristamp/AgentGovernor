...
async function main(): Promise<number> {
    ...
    const { data: orgData } = await admin.request<{ id?: string; organization?: { id: string } }>(
        'POST',
        '/api/auth/organization/create',
        { name: `GCM E2E Org`, slug: uniqueSlug }
    );
    ...
    console.log('   • These map to: filesystem.read_file, filesystem.list_directory, etc.');
    ...
    const claims = decodeJWT(token.accessToken) as { aud?: string; azp?: string; scope?: string; exp?: number; roles?: string[] };
    ...
    try {
        ...
        console.log('⚠️ Should have been denied (JWT still valid without active check)');
    } catch (e) {
    ...
    for (const entry of auditLog.slice(-5)) {
        ...
        console.log(`   ${status} ${entry.tool} - ${entry.identityId ?? 'anonymous'} (${entry.latencyMs}ms)`);
        if (entry.error) {
